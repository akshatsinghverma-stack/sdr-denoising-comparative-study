...
- **Multiple independent training draws.** Monte Carlo trials in both case studies vary the *test* noise and bit sequence; the CNN/LMS/Hybrid models themselves were each trained exactly once per modulation per case study. The variance contributed by the training draw itself (as opposed to test-time randomness) is currently unknown and would need training repeated across several seeds to quantify.
