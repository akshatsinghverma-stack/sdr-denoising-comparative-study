...
**Run on Case Study 2's exact channel at full scale: BPSK tracks MMSE closely at every SNR (both reach zero errors at 15-20dB); QPSK shows a clean, monotonic ordering that sharpens Section 4.7's existing finding rather than contradicting it.** At 20dB: No-Processing 0 errors, MMSE 923/2,000,000, **ZF 1,055/2,000,000 — ZF is the worst of the three**, exactly the textbook prediction (the less a linear equalizer accounts for noise, the more it pays in QPSK's tighter margins for fully inverting a channel that doesn't reward it). This is not a new, independent mechanism — it is the same linear-equalizer noise-enhancement tradeoff from Section 4.7, now shown to scale monotonically with how noise-blind the equalizer is (ZF > MMSE > adaptive/learned methods, in order of how much this specific cost is paid). Full detail: `report/findings_zf_comparison.md`.
...
|---|---|---|
| Clean/near-memoryless channel (negligible multipath), any modulation | **No-Processing (do nothing)** | Case Study 1: every method is *strictly worse* than doing nothing, 56/56 comparisons (Section 3.5) — the raw sample is already the Bayes-optimal decision variable. Adding a filter here is pure downside. |
...
| ISI channel, need a non-adaptive closed-form reference and channel is knowable | **MMSE over ZF** | Section 4.9: ZF (no noise-awareness) is consistently the worst of the three linear methods (No-Processing/MMSE/ZF) for QPSK at moderate-to-high SNR — MMSE's partial noise-awareness is strictly better than ZF's full channel inversion whenever noise is non-negligible. |

**If forced to name one single default recommendation**: **a CNN denoising autoencoder, deployed only when the channel has real multipath/ISI structure and the target platform has at least applications-processor-class compute.** This is the one method in this whole project that never *loses* to No-Processing once real channel structure exists (Case Studies 2 and 4), and its advantage grows, not shrinks, as the situation gets more realistic (more crowded modulations, more severe ISI — Sections 5 and 7). Its two real weaknesses are both precisely characterized rather than hand-waved: it actively hurts on a memoryless channel (Section 3.5 — so it should never be blindly deployed without confirming the channel actually has memory), and it has a real, structural, partially-understood high-SNR error floor (Section 3.7) that is a legitimate open engineering problem, not a fatal flaw.
...
7. **A regression test suite (`tests/`, 58 tests) now codifies every invariant found this way** — LMS/NLMS never diverging, `receiver_frontend` recovering unit gain, No-Processing's SNR matching nominal, `add_multipath` being causal and identity-preserving, no train/test bit-sequence collisions, the genie MMSE never losing to No-Processing — so a future change to this codebase can't silently reintroduce any of the three bugs documented in Section 4.2, or the divergence bug from Section 3.2, without a test failing.
...
13. **The same discipline made partial progress on a second previously-open question — and, honestly, hit a negative result on the proposed fix.** Section 3.7's high-SNR CNN error floor was left as "not root-caused, two guesses offered." Position-level tracking (`report/findings_cnn_high_snr_floor.md`) found the window/overlap-add reconstruction artifact hypothesis strongly evidenced (a statistically significant, χ²=168.0 p=3.5×10⁻³⁶, non-uniform error clustering by window phase) and the competing "unfixable noise" hypothesis not supported. The named intervention — a triangular overlap-add reweighting — was then actually implemented and tested, and did *not* close the floor (error counts unchanged for BPSK, unchanged-to-very-slightly-worse for QPSK). Reporting a tested fix that didn't work is as important as reporting one that did (Section 6.4/Section 4.8) — the *where* of this floor is confirmed; a working fix for it is not.
...
16. **A missing standard baseline (Zero-Forcing) was found by searching comparable published work, not by re-reading this project's own code.** ZF, added alongside the existing genie-aided MMSE bound (`report/findings_zf_comparison.md`, Section 4.9), confirmed the textbook prediction directly: at moderate-to-high SNR, ZF (which ignores noise entirely) is consistently the worst of the three linear methods for QPSK, sharpening rather than contradicting Section 4.7's existing MMSE-regression finding — the less noise-aware a linear equalizer is, the more it pays in a crowded modulation's tight decision margins.
...
- ~~Zero-Forcing (ZF) as a linear-equalizer baseline~~ — **done**, `report/findings_zf_comparison.md` (Section 4.9): found by searching comparable published literature rather than re-reading this project's own code. Confirms and sharpens Section 4.7's QPSK-MMSE-regression finding: ZF (no noise-awareness) is the worst of the three linear methods at moderate-to-high SNR, a monotonic extension of the same noise-enhancement tradeoff.
