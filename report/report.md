...
(Differences of this size at 100,000 symbols × 10 trials — e.g. 198→81, 8093→4371 — are far beyond any plausible trial-to-trial noise, not a marginal or ambiguous shift.) **"vs. LMS/NLMS(after)" is ≥1.0 at every single SNR level tested** — RLS with tail-averaging is never worse than LMS/NLMS anywhere in this study's range, confirming the original future-work rationale ("RLS should show a real edge on this correlated-input channel") once RLS is given the same hardening LMS/NLMS already had. The unhardened RLS's low-SNR shortfall was an implementation gap, not a fundamental property of the algorithm.
...
|---|---|---|
| Clean/near-memoryless channel (negligible multipath), any modulation | **No-Processing (do nothing)** | Case Study 1: every method is *strictly worse* than doing nothing, 56/56 comparisons (Section 3.6) — the raw sample is already the Bayes-optimal decision variable. Adding a filter here is pure downside. |
